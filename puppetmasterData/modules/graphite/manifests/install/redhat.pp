...
class graphite::install::redhat {
	...
	exec {
		'Install django-tagging':
			...
			unless  => "test -d /usr/lib/python2.6/site-packages/django_tagging-0.3.1-py2.6.egg",
			require => Anchor['graphitepkg::end'];
		'Install twisted':
			command => 'easy_install twisted==11.1.0',
			cwd     => "${::graphite::params::build_dir}",
			unless  => "test -d /usr/lib/python2.6/site-packages/Twisted-11.1.0-py2.6-linux-x86_64.egg",
			require => Anchor['graphitepkg::end'];
		'Install txamqp':
			command => 'easy_install txamqp==0.4',
			cwd     => "${::graphite::params::build_dir}",
			unless  => "test -d /usr/lib/python2.6/site-packages/txamqp",
			require => Anchor['graphitepkg::end'];
	}
